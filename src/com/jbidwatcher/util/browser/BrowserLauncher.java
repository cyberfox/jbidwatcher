 ...
 * <br>Minor changes by Morgan Schweers to better support Windows and post-3.x (mrj.version > 100) MRJ versions.
 *
...
public class BrowserLauncher {
    ...
    static {
        ...
        if (osName.startsWith("Mac OS")) {
            ...
            //  If it's in the form of 'm.n', or a 3-digit number (big version number!), this'll work.
            if((mrjVersion.length() > 3) && (mrjVersion.indexOf('.') == 1)) {
              majorMRJVersion = mrjVersion.substring(0, 3);
            } else {
              //  Treat the post-Panther JRE version numbers (99 and above) as 3.1 or later.
              majorMRJVersion = "3.1";
            }
            try {
        ...
        switch (jvm) {
            case MRJ_2_0:
                try {
                    ...
                    // me know.
                    return (String)appleEventConstructor.newInstance(gurlType, gurlType, aeTarget, kAutoGenerateReturnID, kAnyTransactionID);
                } catch (IllegalAccessException iae) {
                    browser = null;
                    errorMessage = iae.getMessage();
                    return browser;
    ...
    public static void openURL(String url) throws IOException { openURL(url, null, false); }
    public static void openURL(String url, String optlaunch) throws IOException { openURL(url, optlaunch, false); }
    /**
            ...
            case WINDOWS_9x:
              ...
              if(process == null) {
                if(optlaunch != null) {
                  ...
                  // characters to work.
                  Runtime.getRuntime().exec(new String[] {browser, FIRST_WINDOWS_PARAMETER,
              ...
              break;
            case OTHER:
              //  IF the user hasn't provided an 'optional launcher',
              //  presume that we're on Unix and that Netscape is
              //  installed
              if(optlaunch != null) {
                if(!optlaunch.equals(browser)) {
                  Runtime.getRuntime().exec(new String[] { optlaunch, url } );
                  break;
                }
              }
